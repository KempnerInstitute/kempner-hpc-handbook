...
```
Currently, there are four GPU racks in production at the Massachusetts Green High Performance Computing Center (MGHPCC): one A100 GPU rack and three H100 GPU rack. In the following diagram, both the current racks are shown, in addition to the network core designed for non-blocking communication between GPUs.
